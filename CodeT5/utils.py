...
def get_filenames(data_root, task, sub_task, split=''):
    if task == 'concode':
        ...
        data_dir = '../nl2python_data/'
        train_fn = '{}/train.csv'.format(data_dir)
        dev_fn = '{}/valid.csv'.format(data_dir)
        test_fn = '{}/test.csv'.format(data_dir)
    if split == 'train':
        return train_fn
    elif split == 'dev':
        return dev_fn
...
def read_examples(filename, data_num, task):
    read_example_dict = {
        ...
        'nl2python': read_nl2python_examples
    }
